...
opengeneral action-planes add local --endpoint http://127.0.0.1:4767/mcp
...
```bash
opengeneral action-planes show local
```
...
```bash
opengeneral action-planes remove local
```

### `opengeneral spawn [--persona <persona>] [--name <agent-name>] [--action-plane <name>] [--token-ref <ref> | --token-env <env>]`

Spawn an agent from a persona.

```bash
opengeneral spawn --persona coder --name coder
opengeneral spawn --persona coder --name prod-coder --action-plane prod --token-env OPENGENERAL_CODER_PROD_TOKEN
```
...
- Create a Streamable HTTP MCP client to the Action Plane using the configured Bearer token.
...
Agents:
  coder  coder-a1b2c3d4e5f6  coder  local
```
...
```json
{
  "agents": {
    "coder": {
      ...
      "persona": "coder",
      "action_plane": "local",
      "token_ref": "env:OPENGENERAL_CODER_TOKEN"
    }
  }
}
```
...
- `token_ref` tells OpenGeneral where to resolve the Bearer token.
- The Action Plane derives identity from the Bearer token; OpenGeneral does not self-authorize.
